...
function normalizeMovieFromAPI(movie) {
  ...
  const rawShowtimes = Array.isArray(movie.showtimes)
    ? movie.showtimes
    : typeof movie.showtimes === "string"
      ? movie.showtimes.split(",")
      : [];

  const normalized = normalizeMoviePayload({
    title: movie.title ?? movie.name ?? "",
    description: movie.description ?? movie.desc ?? "",
    duration: movie.duration ?? movie.runningTime ?? 0,
    imageUrl: movie.imageUrl ?? movie.image ?? movie.poster ?? "",
    trailerUrl: movie.trailerUrl ?? movie.trailer ?? movie.videoTrailer ?? "",
    showtimes: rawShowtimes,
    isNowShowing: movie.isNowShowing ?? movie.nowShowing ?? true,
    isHot: movie.isHot ?? movie.hot ?? false
  });

  return {
    ...normalized,
    id: String(movie.id ?? movie.movieId ?? fallbackId),
...
export function AppStoreProvider({ children }) {
  ...
  useEffect(() => {
    ...
    async function syncFromAPI() {
      ...
      const [movieResult, userResult] = await Promise.all([fetchMoviesFromAPI(), fetchUsersFromAPI()]);
      if (!isMounted) {
        return;
      }
      ...
      if (userResult.ok && userResult.data.length > 0) {
        const normalizedUsers = userResult.data
          .map(normalizeUserFromAPI)
          .filter((user) => user.email);
        if (normalizedUsers.length > 0) {
          setUsers(ensureUsersHaveAdmin(normalizedUsers));
        }
      }
    }
